...
function normalizeState(data) {
  ...
  normalized.absences = normalized.absences.map(absence => ({
    ...
    periods: Array.isArray(absence.periods) ? absence.periods : periodLabels()
  }));
...
function addPeriod() {
  const nextNumber = state.periods.length + 1;
  state.periods.push({
    id: crypto.randomUUID(),
...
byId("addPeriodBtn").addEventListener("click", addPeriod);
byId("copyDayBtn").addEventListener("click", copyDayLessons);
