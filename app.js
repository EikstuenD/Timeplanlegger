// Fikset versjon: mer robust utfylling av nedtrekksfelt og fraværsvalg.
...
function normalizeState(data) {
  ...
  normalized.absences = normalized.absences.map(absence => ({
    ...
    periods: Array.isArray(absence.periods) ? absence.periods : PERIODS
  }));
...
function teacherLoad(name, day) {
  ...
  return {
    ...
    free: PERIODS.length - busyPeriods.size
  };
}
...
function nearestLessonFor(name, day, period, direction) {
  ...
  const candidates = state.lessons
    ...
    .filter(lesson => direction < 0
      ? PERIODS.indexOf(lesson.period) < index
      : PERIODS.indexOf(lesson.period) > index
    )
    .sort((a, b) => direction < 0
      ? PERIODS.indexOf(b.period) - PERIODS.indexOf(a.period)
...
byId("roomOverviewSelect").addEventListener("change", renderRoomOverview);

byId("absenceDateInput").value = todayDateString();
byId("addAbsenceBtn").addEventListener("click", addAbsence);
byId("absenceRoleSelect").addEventListener("change", () => {
  renderAbsencePersonSelect();
});
byId("absenceDaySelect").addEventListener("change", renderSubstituteNeeds);
byId("substituteDaySelect").addEventListener("change", renderSubstituteNeeds);
byId("workloadDaySelect").addEventListener("change", renderWorkload);
byId("holesDaySelect").addEventListener("change", renderHoles);
byId("holesPeriodSelect").addEventListener("change", renderHoles);
...
byId("findSubBtn").addEventListener("click", showEditSuggestions);

byId("runControlBtn").addEventListener("click", runControl);
...
byId("printBtn").addEventListener("click", () => {
  document.querySelectorAll(".tab").forEach(item => item.classList.remove("active"));
  document.querySelectorAll(".tab-panel").forEach(item => item.classList.remove("active"));
  document.querySelector('[data-tab="absenceTab"]').classList.add("active");
  byId("absenceTab").classList.add("active");
  window.print();
});
