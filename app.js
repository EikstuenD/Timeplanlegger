...
function startEditingLesson(id) {
  ...
  byId("teacherSelect").value = lesson.teacher;
...
function stopEditing() {
  ...
  byId("formHelp").textContent = "Systemet varsler hvis lærer, klasse eller rom allerede er opptatt.";
...
function renderFreeTeachers() {
  ...
  const result = byId("freeTeachersResult");

  const busyLessons = state.lessons.filter(lesson =>
    lesson.day === day && lesson.period === period
  );

  const busyTeacherNames = new Set(busyLessons.map(lesson => lesson.teacher));
  const freeTeachers = state.teachers.filter(teacher => !busyTeacherNames.has(teacher));

  const busyTeacherDetails = busyLessons
    .filter(lesson => lesson.teacher)
    .map(lesson => `${lesson.teacher}: ${lesson.subject} med ${lesson.className} på rom ${lesson.room}`);

  result.innerHTML = "";

  const summary = document.createElement("p");
  summary.className = "summary-line";
  summary.textContent = `${freeTeachers.length} av ${state.teachers.length} lærere er ledige ${day}, ${period}.`;
  result.appendChild(summary);

  const freeTitle = document.createElement("strong");
  freeTitle.textContent = "Ledige:";
  result.appendChild(freeTitle);
  ...
  if (freeTeachers.length === 0) {
    ...
    empty.textContent = "Ingen lærere er ledige i denne timen.";
  ...
  result.appendChild(pillList);

  const busyTitle = document.createElement("strong");
  busyTitle.textContent = "Opptatte:";
  result.appendChild(busyTitle);

  if (busyTeacherDetails.length === 0) {
    const noneBusy = document.createElement("p");
    noneBusy.className = "summary-line";
    noneBusy.textContent = "Ingen lærere er satt opp i undervisning denne timen.";
    result.appendChild(noneBusy);
  } else {
    const busyList = document.createElement("ul");
    busyList.className = "busy-list";
    busyTeacherDetails.forEach(detail => {
      const li = document.createElement("li");
      li.textContent = detail;
      busyList.appendChild(li);
    });
    result.appendChild(busyList);
  }
}
...
byId("freeDaySelect").addEventListener("change", renderFreeTeachers);
byId("freePeriodSelect").addEventListener("change", renderFreeTeachers);
